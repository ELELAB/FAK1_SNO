...
class MyModel(automodel):
   ...
   def user_after_single_model(self):
       self.rename_segments(segment_ids=('B'), renumber_residues=[1])
   ...
   #Use a harmonic potential and X-Y distance group.
        rsr.add(forms.gaussian(group=physical.xy_distance,
                               feature=features.distance(at['CA:417:B'],         # Q417 - Y510 
                                                         at['CA:510:B']),        
                               mean=6.0, stdev=0.1))

        rsr.add(forms.gaussian(group=physical.xy_distance,
                               feature=features.distance(at['CA:482:B'],         # F482 - Y510
                                                         at['CA:510:B']),
                               mean=6.0, stdev=0.1))

        rsr.add(forms.gaussian(group=physical.xy_distance,
                               feature=features.distance(at['CA:524:B'],         # L524 - M529
                                                         at['CA:529:B']),
                               mean=6.0, stdev=0.1))
...
a = MyModel(env,
           ...
           knowns   = '2j0kB_final_tot_clean',               # codes of the templates
           sequence = 'FAK1_2j0kB_fl')               # code of the target -> same name of the alignment file
            
a.starting_model= 1                 # index of the first model
a.ending_model  = 100              # index of the last model - for testing set to 2
